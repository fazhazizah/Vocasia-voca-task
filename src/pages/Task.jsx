...
const Task = ({name}) => {
  ...
  ]);  // Dummy tasks added here
  const [taskInput, setTaskInput] = useState('');
  const [completedTasks, setCompletedTasks] = useState([
    "Prepare for the upcoming exam", // Dummy completed tasks
    "Reply to emails"
  ]);  // Dummy completed tasks added here
    ...
    <div className="min-h-screen bg-gray-900 text-white flex p-8 space-x-8">
      <div className="bg-gray-800 w-1/4 p-6 rounded-lg flex flex-col items-center space-y-4">
        ...
        <h2 className="text-lg font-semibold text-center">
          Welcome Back, <span className="text-purple-400">{name || 'User'}</span>!
        </h2>
        <button className="bg-gray-700 text-gray-300 px-4 py-2 rounded-lg mt-2 hover:bg-gray-600"
        onClick={()=> navigate('/profile', {state: {name}})}
        >
          Edit Profile
        </button>
        <button 
